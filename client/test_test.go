...
func Put(cfg *config, ck *Clerk, key string, value string, log *OpLog, cli int) {
	start := time.Now().UnixNano()
	ck.Put(key, value)
	DPrintf(1111, "Call client %d Put, key is %v and value is %v", ck.clientId, key, value)
	end := time.Now().UnixNano()
	cfg.op()
	if log != nil {
		log.Append(porcupine.Operation{
...
func Append(cfg *config, ck *Clerk, key string, value string, log *OpLog, cli int) {
	start := time.Now().UnixNano()
	DPrintf(1111, "Call client %d Append, key is %v and value is %v", ck.clientId, key, value)
	ck.Append(key, value)
	end := time.Now().UnixNano()
	cfg.op()
	if log != nil {
...
func GenericTest2(t *testing.T, part string, nclients int, nservers int, unreliable bool, crash bool, partitions bool, maxraftstate int, randomkeys bool) {
	...
	for i := 0; i < 3; i++ {
		...
		go spawn_clients_and_wait(t, cfg, nclients, func(cli int, myck *Clerk, t *testing.T) {
			...
			DPrintf(1111, "basic test succeeds....")
