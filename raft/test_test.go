...
func TestRejoin2B(t *testing.T) {
	...
	leader1 := cfg.checkOneLeader()
	cfg.disconnect(leader1)
	DPrintf("[%d] leader disconnected with commitIndex=%d", leader1, cfg.rafts[leader1].commitIndex)
	...
	leader2 := cfg.checkOneLeader()
	cfg.disconnect(leader2)
	DPrintf("[%d] leader disconnected with commitIndex=%d", leader2, cfg.rafts[leader2].commitIndex)

	// old leader connected again
	cfg.connect(leader1)
	DPrintf("[%d] leader connected again with commitIndex=%d", leader1, cfg.rafts[leader1].commitIndex)

	cfg.one(104, 2, true)

	// all together now
	cfg.connect(leader2)
	DPrintf("[%d] leader connected with commitIndex=%d", leader2, cfg.rafts[leader2].commitIndex)
